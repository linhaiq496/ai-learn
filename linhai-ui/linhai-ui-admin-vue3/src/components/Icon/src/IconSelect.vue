...
import { ASSET_ICON_PREFIX, assetIconNames } from './assetIcon'
...
const iconList = ref({
  ...IconJson,
  [ASSET_ICON_PREFIX]: assetIconNames
})
...
function handleClick({ props }) {
  ...
  const firstIcon = iconList.value[currentActiveType.value]?.[0]
  if (!firstIcon) {
    icon.value = ''
    emit('update:modelValue', '')
    return
  }
  emit('update:modelValue', currentActiveType.value + firstIcon)
  icon.value = firstIcon
}
